...
function modifier_mk_passive_stacks:OnAttackLanded(params)
    ...
    if mod then
        ...
        elseif mod:GetStackCount() == 8 then
            mod:SetStackCount(1)
        end
    end
end
...
function modifier_mk_stack_debuff:OnCreated()
    ...
    self.pfxNumber = ParticleManager:CreateParticle("particles/units/heroes/hero_monkey_king/monkey_king_quad_tap_stack.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent())
