...
function modifier_item_sanya_flask_upgrade_1:GetModifierConstantHealthRegen()
    return self:GetAbility():GetSpecialValueFor('hp_regen') or 10.0
end
...
function modifier_item_sanya_flask_upgrade_1:OnCreated()
    ...
    local pfx = ParticleManager:CreateParticle('particles/econ/items/pugna/pugna_ti9_immortal/pugna_ti9_immortal_netherblast_flash_b.vpcf', PATTACH_ABSORIGIN, unit)
...
function modifier_item_sanya_flask_upgrade_1:OnTakeDamage(params)
    ...
    if params.unit == self:GetParent() then
        if params.damage > 0 then
            self:Destroy()
            local pfx = ParticleManager:CreateParticle('particles/items4_fx/spirit_vessel_damage_blast_burst.vpcf', PATTACH_ABSORIGIN, params.unit)
            ParticleManager:ReleaseParticleIndex(pfx)
        end
    end
end
