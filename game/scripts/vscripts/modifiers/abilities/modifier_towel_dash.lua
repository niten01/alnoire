modifier_towel_dash = class({})

function modifier_towel_dash:IsHidden() return true end

function modifier_towel_dash:IsPurgable() return false end

function modifier_towel_dash:DeclareFunctions()
    return {
        MODIFIER_PROPERTY_OVERRIDE_ANIMATION
    }
end

function modifier_towel_dash:GetOverrideAnimation()
    return ACT_DOTA_CAST_ABILITY_1
end

function modifier_towel_dash:OnCreated()
...
function modifier_towel_dash:UpdateHorizontalMotion(me, dt)
...
function modifier_towel_dash:UpdateVerticalMotion(me, dt)
    local info = self.info
    if not info then return end
...
function modifier_towel_dash:OnDestroy()
    if IsServer() then
