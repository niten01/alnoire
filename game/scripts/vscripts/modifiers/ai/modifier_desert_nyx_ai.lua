modifier_desert_nyx_ai = class({})

function modifier_desert_nyx_ai:IsHidden() return true end

function modifier_desert_nyx_ai:IsPurgable() return false end

function modifier_desert_nyx_ai:DeclareFunctions()
    return {
        MODIFIER_EVENT_ON_DEATH
    }
end

function modifier_desert_nyx_ai:CheckState()
    return {
        [MODIFIER_STATE_DISARMED] = true,
    }
end

function modifier_desert_nyx_ai:OnDeath(params)
...
function modifier_desert_nyx_ai:OnCreated()
    ...
    self.impale = unit:FindAbilityByName('nyx_assassin_impale')
    if kv then
...
function modifier_desert_nyx_ai:OnIntervalThink()
    local unit = self:GetParent()
    ...
    if beaconState == 'aggro' and target and target:IsAlive() then
        ...
        if timeInAggro >= requiredTimeInAggro and (currentTime - unit.lastCastTime) >= requiredDelay then
            if CastAbility(unit, target, "nyx_bury") then
                return
            end
            if unit.nyxIsUnderground then
                if CastAbility(unit, target, "nyx_assassin_impale") then
                    return
                end
            end
        end
